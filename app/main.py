...
@app.get("/b23/{vid}")
async def bilibili_redirect(request: fastapi.Request, vid: str) -> fastapi.responses.Response:
...
@app.get("/{bvid}")
@app.get("/video/{bvid}")
async def bilibili_direct(request: fastapi.Request, bvid: str) -> fastapi.responses.Response:
    if "Discordbot" not in request.headers.get("User-Agent", ""):
